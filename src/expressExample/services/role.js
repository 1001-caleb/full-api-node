...
class RoleService {
  ...
  constructor (args = {}) {
    ...
    if (!ROLE_IDS.includes(`${id}`)) { throw new httperrors.BadRequest('Role ID not allowed') }
  ...
  async saveRole () {
    if (!ROLE_NAMES.includes(this.#name)) { throw new httperrors.BadRequest('Role name not allowed') }
